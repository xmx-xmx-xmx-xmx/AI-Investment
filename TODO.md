...
- [x] **D6. 雷达观测表（隔离区状态机）** ✅
  ...
  - [x] `src/radar.py`：`scan_radar()` — 逐只抓历史价格 → 算偏离度/趋势/信号 → 写回飞书
  - [x] 简报注入：早间 `morning` + 收盘前 `closing` 注入「🔭 雷达扫描」区块 + LLM 轻度确认
  - [x] 双信号：抄底（10日-5% 🟡 / 20日-8% 🔵）+ 追涨（5日连续阳线 + 未溢价 🟢）
  - **依赖**：无
...
- [ ] **D6.5 雷达深度分析** — 对有信号的标的用 Tavily/华尔街见闻搜索相关新闻，LLM 输出 2-3 句展开分析。等 D6 轻度信号确认跑通后再决定
  - **依赖**：D6 雷达观测表 + 实际体验验证
