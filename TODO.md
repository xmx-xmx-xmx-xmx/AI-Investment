...
| **1b** | 🔴 **修底仓表被覆盖的 2 行份额**（`C 278.23 → 28.23`、`E 16.22 → 293.70`） | 用户 1 min | **越早越好，理想是下一次运行之前**。9/18 还有 1 笔 ¥500 买入 pending，结算时会以 E 行的**当前值**为基数累加 —— 不先修，E 会变成 `16.22+≈173`，而不是正确的 `293.70+≈173`。修完份额后 市值/收益率/涨跌幅 会被 `price_updater` 自动重算。证据与推导见 **§1.5**；记录 ID：C=`recvlPMQ1z9YGh`、E=`recvvqUxr0Nijk`（⚠️ 底仓表，非流水表） |
