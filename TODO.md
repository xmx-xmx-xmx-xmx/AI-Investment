...
| `pending_resolver.py` 行数 | 886（本次 +convert 分支后） |
| 测试 | **203 用例**，覆盖 8 个模块（macro_calendar / market_data / global_news / radar / strategy / briefing-diff / **convert**）；实测 `pytest -q` 全绿 |
...
**第 4 笔：46.93 买入已入表，但产品类别存疑**（2026-09-17 15:30 补录）

| record_id | 买卖方向 | 产品名称 | 交易金额 | 交易时间 | 状态 |
|---|---|---|---|---|---|
| `recvvsAN850K7v` | buy | …ETF联接**(QDII)C** | 46.93 | 09-17 12:51:46 | pending |

⚠️ **用户描述为"E 类买入"，但表内产品名是 C 类**。若确为 E 类，必须在净值回填前改掉 `产品名称`（改成 `…（QDII）E`）。

原因：`pending_resolver` 按 `产品名称` 模糊匹配底仓（该行 `标的代码` 为空），
C 类会命中 `017093`（用户正在**转出**的那个类别），E 类才会命 `019118`。
两类别净值略有差异（9/15：C 2.8131 / E 2.8386），回填后份额会错，且会往"正在清空的类别"里加份额。
...
| 预计 9/18 晚 ~ 9/19 | 9/17 净值入库 → 两腿同时成交 → 写底仓（C `308.23`→**28.23** 份 / E `0`→**≈277** 份）+ 流水置 `completed` | ⏳ 待触发 |
...
**🔍 待人工核对的小事**
- `docs/CONVERT_DESIGN.md` §1.5 与提示词 few-shot 示例里的示例单号写作 `202609170100…`，表内真实值为 `202609170010…`（第 10-11 位 `10`↔`01` 互换）。**不影响运行**（实测 3 笔单号均被正确 OCR，模型没有照抄示例），已顺手改正。
...
| **0b** | **核对 46.93 那笔的产品类别** | 5 min（+ 用户核对） | 用户 9/17 补录成功（`recvvsAN850K7v`），但**表内产品名是 C 类、用户描述是 E 类**。两者会命中不同底仓（C→`017093` / E→`019118`），净值回填前必须确认。详见 §1.2 |

> ✅ **P0 #0 已闭环（2026-09-17）**：表结构已改 · 代码已改 + 28 用例已过 · 代码已 push · 快捷指令已改完 · **真机录 3 笔成功入表**。
> 全量测试 `203 passed`。完整实测记录、后续自动回填时间表与两条固化约束见 **§1.2**。
