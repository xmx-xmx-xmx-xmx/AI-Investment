...
| 测试 | **318 用例**，覆盖 13 个模块（macro_calendar / market_data / global_news / radar / strategy / briefing-diff / **hard-signals** / **briefing-snapshot** / convert / holding-match / **resolver-batch** / **sector-brief** / **macro-brief**）；实测 `pytest -q` 全绿 |
...
### 1.9 顺手记下的两个周报质量缺陷（**未修**，待排）

1. **LLM 前言泄漏**：9/20 周报正文里出现 `好的，这是根据您的投资宪法和市场信息生成的周报。` —— 收尾指令不够硬。
2. **思维链整段进卡片**：`CHAIN_OF_THOUGHT` 里写着"输出时体现思考过程"（`prompt_templates.py:80`），所以周报/周末复盘卡片里带着 `🧠 顾问推理过程` / `**思考过程：**` 整段。**这是设计如此，不是 bug**，但它是周报篇幅的大头之一，且里面夹着 `****固收 (偏离 +17.24%)****` 这类四星号破损 markdown（TODO P0 #2 已记）。**要不要把思维链从推送卡片里摘掉、只在"想看推理"时输出，需要用户拍板。**
