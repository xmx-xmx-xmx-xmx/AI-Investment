...
**🔍 待人工核对的两件小事**
1. 用户提到"中午另有一笔 **46.93 的 E 类买入**"—— **表里查无此行**（91 行全量核对，金额/单号/产品名三路搜索均无命中；底仓 E 类仍 `份额 0 / 成本 0`）。见 §2 P0 #0b。
2. `docs/CONVERT_DESIGN.md` §1.5 与提示词 few-shot 示例里的示例单号写作 `202609170100…`，表内真实值为 `202609170010…`（第 10-11 位 `10`↔`01` 互换）。**不影响运行**（实测 3 笔单号均被正确 OCR，模型没有照抄示例），已顺手改正。
...
| **0b** | **查清"46.93 的 E 类买入"去向** | 10 min | 用户 9/17 中午另有一笔 E 类买入，但**表里查无此行**（91 行全量核对，金额/单号/产品名三路搜索均无命中；底仓 E 类仍 `份额 0 / 成本 0`）。要么快捷指令那次 POST 失败了（用户以为成功），要么根本没录。需要用户提供支付宝截图核对 |
