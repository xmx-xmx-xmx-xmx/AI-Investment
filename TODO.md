...
- [x] **D2-1 买/卖记录 iPhone 快捷指令（需手动在 iPhone 完成）**
  - [x] 长按 Action Button → 弹出菜单「记一笔」
  - [x] 选「记一笔」→ 系统截图
  - [x] 快捷指令内置「提取文本」功能获取图片内粗糙文本（本地、免费、秒出）
  - [x] 将纯文本 POST 给云端函数
  - [x] 云端返回 JSON 解析成功后 → 快捷指令弹窗提示「已记录：XX基金 买入 ¥100」

- [x] **D2-2 云端纯文本 LLM 入口（`src/cloud_handler.py`）**
  - [x] 接收 POST 的纯文本（非图片）
  - [x] 调用纯文本 LLM 提取 JSON
  - [x] 写入飞书「交易流水表」
  - [x] 返回确认 JSON 给快捷指令
